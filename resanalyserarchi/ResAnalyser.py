...
duplication_issues = {'BUILDING DESIGN AND DRAWING':'BUILDING DESIGN DRAWING',
                      ...
                      'PHYSICS - I':'PHYSICS','PHYSICS I':'PHYSICS',
    ...
    def get_batch(roll,stud_type):
        if roll[:2] == 'AR':
            if latest_terms[0][0] == "S":
                return str(int(latest_terms[0][-4:])-1)
            return latest_terms[0][-4:]
        else:
            return roll[:4]
            ...
            if cur_data:
                if cur_data.split()[0] in terms:
                        ...
                        if cur_data in grades and is_no_of_credit(cur_block[-2]):
                            ...
                            if good_to_add:
                                ...
                                if course in course_data:
                                    ...
                                    course_data[course] = {'Serial':serial,'Records':{},'Branches':[cur_branch]}
                                    course_data[course]['Records'][cur_term] = {}
                                    course_data[course]['Records'][cur_term][cur_roll] = cur_grade
                    ...
                    if is_gpa(cur_block[-2]) and not len(cur_block) < 7:
                        if cur_roll == 'BA09ARC051':
                            print(all_details,cur_block)
...
class Analyser:
    ...
    def Make_Marklist(course=False,course_term=None,multi=False,branch=None,batch=None,term=latest_terms[0],cg=False,sg=False,names=False):
        mark_list = []
